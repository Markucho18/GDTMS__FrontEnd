...
export function TareasCompletas() {
  ...
  const getTareas = () => {
    axios
      .get(`http://localhost:3001/tareas/completas?userId=${userId}`)
      .then((res) => {
        if (res) {
  ...
  const limpiarTareas = () => {
    ...
    if (confirmar === true) {
      axios
        .delete(`http://localhost:3001/tareas/completas?userId=${userId}`)
        .then((res) => {
          if (res) actualizarTareas();
          else
            console.log("No hubo respuesta limpiarTareas() desde el backend");
        })
        .catch((err) =>
  ...
  return (
    <ul className="flex flex-col gap-1 overflow-y-auto">
      <div className="flex items-center justify-center">
        ...
        <p>
          <i
            className="fa-solid fa-delete-left flechita"
